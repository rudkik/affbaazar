...
def main():
    ...
    # маскировка ссылок: точка в скобках, словом, пробелами, юникод-точки, невидимые символы
    bad("пиши example[.]com", why="[.]")
    bad("пиши example(dot)com", why="(dot)")
    bad("сайт example{точка}com", why="{точка}")
    bad("сайт example dot com", why="dot словом")
    bad("сайт пример точка рф", why="точка словом")
    bad("сайт example . com", why="пробелы вокруг точки")
    bad("канал t․me/spam", why="U+2024")
    bad("канал t．me/spam", why="U+FF0E")
    bad("канал t.m​e/spam", why="zero-width внутри")
    bad("канал t​.me/spam", why="zero-width перед точкой")
    bad("ссылка bit.ly/xyz", why="bit.ly")
    # нормализация не должна плодить ложных срабатываний
    ok("Пишите . Отвечу всем", why="точка с пробелами между обычными словами")
    ok("Опыт (5 лет) . Работаю удалённо", why="точка после скобки")
    ok("Первое . Второе . Третье", why="перечисление через точки")
    ok("dot com — это не домен без слова слева", why="dot в начале")
    print("маскировка ссылок OK: [.] (dot) «точка» пробелы, юникод-точки, zero-width")
