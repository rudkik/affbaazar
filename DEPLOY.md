...
Коины продаются только за криптовалюту (USDT/USDC через CryptoPay). Покупка за Telegram Stars
убрана: старые кнопки «N коинов — M ⭐» и ранее выставленные счета в звёздах бот отклоняет.
Для уже прошедших Stars-платежей остался возврат: `/refund <charge_id>` (charge_id виден в
админке во вкладке «Платежи»). Переменная `PAYMENT_PROVIDER_TOKEN` больше не используется.

### Крипта: USDT/USDC через CryptoPay

Процессинг `https://ubaduba.top` (описание API — `INTEGRATION.md`). Когда в `.env` задан
`CRYPTOPAY_API_KEY`, в разделе «Баланс / Купить коины» появляются пакеты за USDT/USDC: бот выставляет
счёт, даёт ссылку на страницу оплаты (QR, выбор сети Tron/BSC/Ethereum) и начисляет коины по
вебхуку после подтверждения в сети. Кнопка «Проверить оплату» под счётом перечитывает его через
API — страховка, если вебхук не дошёл.

Подключение:

1. В админке процессинга (Services → сервис этого проекта) взять **API-ключ** `cp_live_…` и
   **Webhook secret**, положить в `.env`: `CRYPTOPAY_API_KEY`, `CRYPTOPAY_WEBHOOK_SECRET`.
2. В карточку сервиса вписать адрес вебхука: **`https://affbazaar.com/webhooks/cryptopay`**
   (`${PUBLIC_URL}/webhooks/cryptopay`).
3. `make up` (перезапуск бота, чтобы подхватить `.env`), затем в боте: «Баланс / Купить коины» →
   «Оплатить криптой» → счёт → оплата → сообщение «Оплата получена».

Пакеты — настройка `crypto_packages` в админке: `[{"usd": "5", "tokens": 50}, …]`.
Переплата и частичная оплата зачисляются пропорционально пакету; реверс платежа (реорг сети)
списывает начисленное обратно. Оплаты видны во вкладке «Платежи» (Charge ID = id счёта).
Вебхук с неверной подписью получает `401`, повторные доставки (Retry в админке процессинга)
не начисляют второй раз. Логи: `make logs | grep cryptopay`.
...
5. «Запрет дублей» (`ad_dup_hours`, по умолчанию 1 час): одно и то же объявление нельзя
   опубликовать повторно раньше, чем через столько часов, — бот скажет, через сколько минут можно.

## Бот поддержки

Второй бот (@aff_bazaar_support_bot) живёт в том же процессе: пользователь пишет ему в личку,
на него создаётся топик в форум-группе поддержки, ответы из топика уходят пользователю.
В топике работают `/ban` и `/unban`. В главном боте кнопка «🆘 Поддержка» ведёт по ссылке из
настройки `support_link` (админка сайта → «Настройки»; пусто = кнопки нет).

Подключение:

1. В @BotFather создать бота поддержки, токен положить в `.env`: `SUPPORT_BOT_TOKEN`.
2. Создать супергруппу, включить в ней **Топики** (Settings → Topics), добавить бота поддержки
   администратором с правом **управлять топиками** (Manage Topics) и писать сообщения.
3. ID группы положить в `SUPPORT_CHAT_ID` (вида `-100…`; подойдёт и номер из адресной строки
   Telegram Web, префикс `-100` бот добавит сам).
4. `make up`. В логах должна быть строка «Бот поддержки запущен: @… → группа …».
5. Проверка: написать боту поддержки с обычного аккаунта → в группе появился топик с карточкой
   пользователя и его сообщением; ответить в топике → ответ пришёл пользователю.
...
| Счёт на оплату не выставляется | Смотрите `make logs \| grep cryptopay`: `forbidden` — сервис выключен в админке процессинга, `unauthenticated` — не тот `CRYPTOPAY_API_KEY`, `wallet_not_configured` / `no_free_address` — в процессинге не настроены адреса приёма |
