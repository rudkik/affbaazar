...
`CRYPTOPAY_API_KEY`, в меню «Купить коины» появляется кнопка «Оплатить криптой»: бот выставляет
...
3. `make up` (перезапуск бота, чтобы подхватить `.env`), затем в боте: «Купить коины» →
